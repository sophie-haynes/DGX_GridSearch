...
def run_model_training(model, bsz, lr, momentum, seed, pos_class_weight, target_mom, target_mom_rate, args):
    ...
    if args.train_set in ext_names:
        ext_names.remove(args.train_set)
    # add support for combined training
    elif args.train_set == "padcxr14":
        ext_names = ['openi', 'jsrt']
    else:
        raise ValueError("Invalid train_set value. Must be one of 'cxr14', 'padchest', 'openi', 'jsrt' or 'padcxr14'.")
    ...
    if args.train_set != "padcxr14":
        ...
        test_loader_list = [testloader, ext1loader, ext2loader, ext3loader]
        test_name_list = ['test1', 'ext1', 'ext2', 'ext3']
    else:
        train_path = os.path.join(args.data_root,"padcxr14", "train")
        ...
        testloader2 = torch.utils.data.DataLoader(testset2, batch_size=bsz, shuffle=False, num_workers=args.num_workers, pin_memory=True)
        ext2loader = torch.utils.data.DataLoader(ext2set, batch_size=bsz, shuffle=False, num_workers=args.num_workers, pin_memory=True)
        ext3loader = torch.utils.data.DataLoader(ext3set, batch_size=bsz, shuffle=False, num_workers=args.num_workers, pin_memory=True)
        test_loader_list = [testloader1, testloader2, ext2loader, ext3loader]
        test_name_list = ['test1', 'test2', 'ext2', 'ext3']
    ...
    metrics_dict = {
        ...
        "{}_f1".format("test2" if args.train_set == "padcxr14" else "ext1"): [],
        "ext2_auc": [],
        "ext2_precision": [],
        "ext2_recall": [],
        "ext2_f1": [],
...
def parse_args():
    ...
    parser.add_argument("--train_set", type=str, required=True, help="Training dataset name (e.g., 'cxr14')", choices = ["cxr14", "padchest","padcxr14"])
