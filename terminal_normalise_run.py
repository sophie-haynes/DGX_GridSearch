...
def run_model_training(crop_size, process, train_set, model, model_name, bsz, lr, momentum, patience, tuning_strategy, log_dr,data_root="/content/", num_epochs=10,num_workers=8, single=False, seed=None,pos_class_weight=1.0):
    ...
    internal_mean = crop_dict[train_set][0]
    internal_std = crop_dict[train_set][1]


    # Data transformations
    internal_train_transform = get_cxr_train_transforms(crop_size, v2.Normalize(mean=internal_dict[train_set][0], std=internal_dict[train_set][1]), single)
    internal_test_transform = get_cxr_eval_transforms(crop_size,v2.Normalize(mean=internal_dict[train_set][0], std=internal_dict[train_set][1]), single)
    ext1_test_transform = get_cxr_eval_transforms(crop_size,v2.Normalize(mean=internal_dict[ext_names[0]][0], std=internal_dict[ext_names[0]][1]), single)
    ext2_test_transform = get_cxr_eval_transforms(crop_size,v2.Normalize(mean=internal_dict[ext_names[1]][0], std=internal_dict[ext_names[1]][1]), single)
    ext3_test_transform = get_cxr_eval_transforms(crop_size,v2.Normalize(mean=internal_dict[ext_names[2]][0], std=internal_dict[ext_names[2]][1]), single)
    ...
    trainset = torchvision.datasets.ImageFolder(root=train_path, transform= v2.Compose(internal_train_transform), target_transform=remap_labels)
    testset = torchvision.datasets.ImageFolder(root=test_path, transform=v2.Compose(internal_test_transform), target_transform=remap_labels)
    ext1set = torchvision.datasets.ImageFolder(root=ext1_path, transform=v2.Compose(ext1_test_transform), target_transform=remap_labels)
    ext2set = torchvision.datasets.ImageFolder(root=ext2_path, transform=v2.Compose(ext2_test_transform), target_transform=remap_labels)
    ext3set = torchvision.datasets.ImageFolder(root=ext3_path, transform=v2.Compose(ext3_test_transform), target_transform=remap_labels)
